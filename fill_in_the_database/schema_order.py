...
def fix_order_dates(df_orders, df_clients):
    df = pd.merge(df_orders, df_clients[['id_client', 'registration_date']],
    ...
    df['order_date'] = df.apply(
        lambda row: row['registration_date'] if row['order_date'] < row[
            'registration_date'] else row['order_date'], axis=1)
...
def add_total_column(df_orders, df_order_details):
    ...
    df_order_details['unit_price'] = df_order_details['unit_price'].replace(r'[\$,]', '',
                                                                            regex=True).astype(
        float)
    ...
    df_totals = (df_order_details.assign(
        total=lambda df: df['unit_price'] * df['amount']).groupby('id_order')[
...
def validate_and_complete_id_order(order_details_csv, orders_csv, output_csv,
                                   product_csv=None):
    # Load files
    df_order_details = pd.read_csv(order_details_csv)
    df_orders = pd.read_csv(orders_csv)
    ...
    if num_invalid > 0:
        ...
        df_order_details.loc[invalid_mask, 'id_order'] = [random.choice(valid_order_ids)
    ...
    print(
        f"🔍 Found {num_missing} id_order values not present in order_details. They will be added.")
    ...
    if product_csv:
        ...
        if not df_products.empty:
            valid_product_id = random.choice(
                df_products['id_product'].astype(int).tolist())

    # Calculate last id_detail
    max_id_detail = df_order_details[
        'id_detail'].max() if not df_order_details.empty else 0

    # Create new rows
    new_rows = []
    for i, id_order in enumerate(missing_orders, start=1):
        new_rows.append({'id_detail': max_id_detail + i, 'id_order': id_order,
            'id_product': valid_product_id, 'amount': 0, 'unit_price': 0})
